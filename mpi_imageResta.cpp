...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < rows_per_process; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            buffer1[index] = std::abs(buffer[index] - buffer1[index]);
            buffer1[index+1] = std::abs(buffer[index+1] - buffer1[index+1]);
            buffer1[index+2] = std::abs(buffer[index+2] - buffer1[index+2]);;
            buffer1[index+3] = 255;
        }
    }
